...
class Creport extends Cezpdf {
	...
	function dots($info){
		$tmp = $info['p']; // draw a dotted line over to the right and put on a page number
		$lvl = $tmp[0];
		$lbl = substr($tmp,1);
		$xpos = 520;
		switch($lvl){
...
$pdf->addText(50,34,6,'http://www.proxitek.com - Web Agency');
$pdf->restoreState();
$pdf->closeObject();
...
$pdf->ezText("FACTURE N°1\n",30,array('justification'=>'centre'));
$pdf->ezText("\n<c:alink:http://www.golfbowl.com>Golfbowl.com</c:alink>",30,array('justification'=>'centre'));

$pdf->addJpegFromFile('../../../imgs/references/references_visu1_050926152601.jpg',199,$pdf->y-300,200,200);
...
if (file_exists('ros.jpg')){
	$pdf->addPngFromFile('../../images/proxitek.png',199,$pdf->y-100,200,0);
} else {
