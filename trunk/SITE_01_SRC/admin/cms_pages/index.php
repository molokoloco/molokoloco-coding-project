...
switch ($mode) { // $R1 -> CAT // $R2 -> PROD
	case 'fiche' :
		...
		elseif ($rub_id > 0) $A = new FICHE($R1,$R1_data,$id);
	...
	case 'bdd' :
		...
		elseif ($rub_id > 0) $A = new BDD($R1,$R1_data,$action,$id);
	...
	case 'cms' :
		require('cms_edit.php');
		/*require('../lib/class/class_admin_cms.php');
		$A = new CMS($R2,$R2_data,$id);
		$A->page_id = $id;
		$A->createFICHE();*/
	...
	default : // 'liste'
		...
		elseif ($rub_id > 0) $A = new LISTE($R1,$R1_data,$id);
