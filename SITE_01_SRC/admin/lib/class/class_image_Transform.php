...
/* CHECK  http://pear.php.net/package/Image_Transform/docs/0.9.1/  */
...
class Image_Transform
{
	...
	function &factory($driver='')
    {
		...
		if ($driver == 'GD') {
			require_once('class_image_GD.php');
			$classname = "Image_Transform_Driver_GD";
		}
		elseif ($driver == 'IM') {
			require_once('class_image_IM.php');
			$classname = "Image_Transform_Driver_IM";
		}
		else return false;

        if (!class_exists($classname)) {
            $this->error = 'Aucun module de traitement d\'image ne semblent être défini';
			return false;
        }
        $obj =& new $classname;
     ...
     *
     * @param  object error info when setting an error
     * @return mixed FALSE or error info
     * @access protected
     */
    function &error($error = null,$errorNumber = null)
    {
        if (!is_null($error)) {
            $this->_error =& $error;
        }
     ...
     * @return mixed TRUE or error info on error
     * @access public
     */
    ...
    {
        if ($new_x <= 0) {
            $this->error = 'New size must be strictly positive';
			return false;
        }
     ...
     * @return bool|PEAR_Error TRUE or error info on error
     * @access public
     */
    ...
    function _get_image_details($image)
    {
        ...
        // 15 = WBMP, 16 = XBM
        if (!is_array($data)) {
            $this->error = "Cannot fetch image or images details";
			return false;
        }
        ...
        switch ($data[2]) {
            ...
            case IMAGETYPE_XBM:
                ...
                break;
            default:
                $this->error = "Cannot recognize image format";
				return false;
        }
    ...
    {
		global $host;
		//if (C_PLATFORM != 'WINDOWS') return $path . $command . ' ' . $args;
        //else return 'start /D "' . $path . '" /B ' . $command . ' ' . $args;
		//if (strpos($path, '.exe')) $path = substr($path,0,-4);
    ...
    function _resize()
    {
        $this->error = 'Resize method not supported by driver';
		return false;
    }
    ...
    function load($filename) {
        $this->error = 'load() method not supported by driver';
		return false;
    }
    ...
    function display($type, $quality = null) {
        $this->error = 'display() method not supported by driver';
		return false;
    }
    ...
    function save($filename, $type, $quality = null) {
        $this->error = 'save() method not supported by driver';
		return false;
    }
     ...
     */
    function free() {
        $this->error = 'free() method not supported by driver';
		return false;
    }
    ...
    function addText()
    {
        $this->error = 'addText() method not supported by driver';
		return false;
    }

    function addDropShadow()
    {
        $this->error = 'addDropShadow() method not supported by driver';
		return false;
    }

    function addBorder()
    {
        $this->error = 'addBorder() method not supported by driver';
		return false;
    }
     ...
     *
     * @return mixed TRUE or a error info on error
     * @access public
     **/
    function crop($width, $height, $x = 0, $y = 0)
    {
        $this->error = 'crop() method not supported by driver';
		return false;
    }
	function cropCenter($width, $height)
    {
        $this->error = 'cropCenter() method not supported by driver';
		return false;
    }
	
	function POLA($maxWidth,$maxHeight) {
		$this->error = 'POLA() method not supported by driver';
		return false;
	}

    function canvasResize()
    {
        $this->error = 'canvasResize() method not supported by driver';
		return false;
    }
    ...
    {
        $this->error = 'gamma() method not supported by driver';
    }
     ...
     * @param mixed $options
     * @return bool|PEAR_Error TRUE on success, error info on error
     * @access public
     */
    function rotate($angle, $options = null)
    {
        $this->error = 'rotate() method not supported by driver';
    }
    ...
    function mirror()
    {
        $this->error = 'mirror() method not supported by driver';
		return false;
    }
    ...
    function flip()
    {
        $this->error = 'flip() method not supported by driver';
		return false;
    }
    ...
    function greyscale()
    {
        $this->error = 'greyscale() method not supported by driver';
		return false;
    }
