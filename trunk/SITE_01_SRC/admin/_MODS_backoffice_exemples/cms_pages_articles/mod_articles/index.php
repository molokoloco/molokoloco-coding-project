...
switch ($mode) {
	...
	case 'fiche' :
	if ($_SESSION[SITE_CONFIG]['ADMIN']['type'] != '3') { // Check USER right // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		$id = gpc('id');
		if ($id > 0) {
			$A =& new Q("SELECT utilisateur_id FROM mod_articles WHERE id='".$id."' LIMIT 1");
			$utilisateur_id = intval($A->V[0]['utilisateur_id']);
			if ($utilisateur_id != $_SESSION[SITE_CONFIG]['ADMIN']['id']) {
				
				$A = new LISTE($R1,$R1_data,$id);
				$A->rub_id = $rub_id;
				$A->cat_id = $cat_id;
				$A->child = $child;
				$A->createLISTE();
