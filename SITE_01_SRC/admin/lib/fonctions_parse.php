...
function cleanWysiwyg($string, $br=1) {
	...
	if (strpos($string,'<br />') !== false) {
		...
		while (strpos(substr($string,0,8), '<br />') !== false) $string = str_replace('<br />', '',substr($string,0,8)).substr($string,8); // BR de d�but
...
function cleanName($string) {
	...
	$special = array('&', 'O', 'Z', '-', 'o', 'z', 'Y', 'A', 'Á', 'Â', 'A', 'Ä', 'A', 'A', 'Ç', 'E', 'É', 'E', 'Ë', 'I', 'Í', 'Î', 'I', 'D', 'N', 'O', 'Ó', 'Ô', 'O', 'Ö', 'U', 'Ú', 'U', 'Ü', 'Ý', 'a', 'á', 'â', 'a', 'ä', 'a', 'a', 'ç', 'e', 'é', 'e', 'ë', 'i', 'í', 'î', 'i', '?', 'n', 'o', 'ó', 'ô', 'o', 'ö', 'u', 'ú', 'u', 'ü', 'ý', 'y', '.', ' ', '+', '\'');
	$normal = array('et', 'o', 'z', '-', 'o', 'z', 'y', 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'c', 'e', 'e', 'e', 'e', 'i', 'i', 'i', 'i', 'd', 'n', 'o', 'o', 'o', 'o', 'o', 'u', 'u', 'u', 'u', 'y', 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'c', 'e', 'e', 'e', 'e', 'i', 'i', 'i', 'i', 'o', 'n', 'o', 'o', 'o', 'o', 'o', 'u', 'u', 'u', 'u', 'y', 'y', '_', '_', '-', '-');
...
function cleanTag($string) { // l\\\'ApostrophE > l'apostrophe > l\\\'apostrophe
	...
	preg_match("/^[a-z0-9&���������������\-' ]{2,150}$/",$string, $matches); // Validate STRING as WORD
...
function stringToRegex($string,$sql=1) { // String is "clean" ("act\\\'ion")
	// WHERE "act\'ion" REGEXP "^[a���][c�]t\\\\'[i��][o��]n[s]?$" 
	...
	$string = preg_replace('|[a���]{1}|', '[a���]', $string); // SQL REGEX
	$string = preg_replace('|[e����]{1}|', '[e����]', $string);
	$string = preg_replace('|[i��]{1}|', '[i��]', $string);
	$string = preg_replace('|[u���]{1}|', '[u���]', $string);
	$string = preg_replace('|[o��]{1}|', '[o��]', $string);
	$string = preg_replace('|[c�]{1}|', '[c�]', $string);
	$string = preg_replace('|[ ]{1}|', '[s]? ', $string); // mots cl� == mot cl�
	...
	if ((strpos($MyDate,'/') == false && strpos($MyDate,'-') == false) && (strlen($MyDate) == 6 || strlen($MyDate) == 8)) {
		...
		if ($lettre == '0') return $j.'/'.$m.'/'.$a;
			...
			if ($lg == 'fr') {
				...
				$dateform = preg_replace("/(\D)1st/","\${1}1er",$dateform); // 1st qui n'est pas pr�c�d� par un chiffre 
...
function printDateTime($dateTime,$lettre=0,$lg='fr') {
	...
	elseif ($lettre == 2) { // Mercredi 31 Janvier 2007 � 10h30
		$date = rDate($a.$m.$j, '1', $lg);
		$time = substr($time, 0, -3);
		$time = str_replace(':', 'h', $time);
		return $date.' &agrave; '.$time;
	}
	elseif ($lettre == 3) { // Mercredi 31 Janvier � 10h30
		$date = rDate($a.$m.$j, '1', $lg); 
