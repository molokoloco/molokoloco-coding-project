...
class Image_Transform_Driver_GD extends Image_Transform
{
    ...
    function __construct()
    {
       $types = ImageTypes();
		...
		if (!$this->_supported_image_types) {
			$this->error($this->error("No supported image types available", IMAGE_TRANSFORM_ERROR_UNSUPPORTED));
		}
    } // End function Image
    ...
    function load($image)
    {
        ...
        if (!$this->supportsType($this->type, 'r')) {
            $this->error = 'Image type not supported for input';
			return false;
        }
        ...
        if (!$this->imageHandle) {
            ...
            $this->error = 'Error while loading image file.';
			return false;
        }
        return true;
    ...
    function crop($width, $height, $x = 0, $y = 0)
    {
        ...
        if (!$this->intersects($width, $height, $x, $y)) {
            return $this->error('Nothing to crop', IMAGE_TRANSFORM_ERROR_OUTOFBOUND);
        }
        ...
        if (!imagecopy($new_img, $this->imageHandle, 0, 0, $x, $y, $width, $height)) {
            ...
            return $this->error('Failed transformation: crop()',
    ...
    */
    function _resize($new_x, $new_y, $options = null)
    {
        if ($this->resized === true) {
            return $this->error('You have already resized the image without saving it.  Your previous resizing will be overwritten', null, PEAR_ERROR_TRIGGER, E_USER_NOTICE);
        }
    ...
    function _generate($filename, $type = '', $quality = null)
    {
        ...
        if (!$this->supportsType($type, 'w')) {
            return $this->error('Image type not supported for output', IMAGE_TRANSFORM_ERROR_UNSUPPORTED);
        }
        ...
        if (!$result) {
            return $this->error('Couldn\'t ' . $action,
    ...
    {
        if (!trim($filename)) {
            return $this->error('Filename missing', IMAGE_TRANSFORM_ERROR_ARGUMENT);
        }
